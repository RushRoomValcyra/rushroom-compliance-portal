...
**Status:** deployed 2026-09-15 (migrations 0025 + 0026 applied, `portal-api` redeployed). Add / remove / re-add confirmed working; reordering not yet confirmed — the `↑`/`↓` controls were gated on `sibCount > 1` and therefore invisible on the single-child assemblies used for testing, fixed in v204 by always rendering them and disabling at the ends.
